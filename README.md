...
Кроме бота в репозитории есть встроенный веб-сервер (Gin) с своим API и
поиском по локальному индексу торрентов (Bleve), синхронизируемому из
внешнего источника метаданных — для запуска **только бота** он не нужен и
в инструкции ниже не описан.
...
|---|---|---|
| `torrs_bot` (этот репозиторий) | сама логика бота, веб | собирается из исходников (Go) |
...
|---|---|---|
| `-p, --port` | `8094` | порт встроенного веб-сервера |
...
| `-r, --rebuild-index` | — | пересобрать поисковый индекс и выйти |
| `--tmdb` | — | включить прокси для TMDB |
...
- `torrents.db`, `index.db/` — локальные базы (кэш метаданных, кэш
  Telegram `file_id`, поисковый индекс);
- `userbot.session` — сессия юзербота, если настроен (шаг 8);
- `.env` — секреты (токен бота, `api_id`/`api_hash`).
